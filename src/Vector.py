...
class CoordinatePlane(Scene):
    def construct(self):
        ...
        text = MathTex("(2,4)", tex_to_color_map={"2": BLUE, "4": YELLOW}).move_to(
            UP * 3.0 + RIGHT * 6.0
        )

        self.add(text)  # pyright: ignore[reportUnusedCallResult]
        self.add(x_axis)
        self.add(y_axis)
        ...
        lines = axes.get_lines_to_point(
            coord.get_center()  # pyright: ignore[reportArgumentType]
        )
        self.play(
            Create(coord),
            Create(lines),
...
class Addition(Scene):
    def construct(self):
        ...
        equation = (
            MathTex(
                "(1,2)+(3,1)=(4,3)",
                tex_to_color_map={"(1,2)": YELLOW, "(3,1)": BLUE, "(4,3)": GREEN},
            )
...
class AdditionDistribute(Scene):
    def construct(self):
        ...
        x_arrow2 = Arrow(
            numberPlane.c2p(1, 2), numberPlane.c2p(4, 2), buff=0, color=BLUE
        )

        y_arrow1 = Arrow(
            numberPlane.c2p(1, 0), numberPlane.c2p(1, 2), buff=0, color=YELLOW
        )
        y_arrow2 = Arrow(
            numberPlane.c2p(4, 2), numberPlane.c2p(4, 3), buff=0, color=BLUE
        )
        ...
        self.play(
            x_arrow2.animate.shift(DOWN * 2.0), y_arrow1.animate.shift(RIGHT * 3.0)
        )
        self.wait(0.5)
...
class Scaling(Scene):
    def construct(self):
        ...
        equation = (
            MathTex("(2,1)*2=(4,2)", tex_to_color_map={"(2,1)": YELLOW})
            .move_to(UP * 3.5 + RIGHT * 5.0)
            .scale(0.8)
        )

        arrow = Arrow(
            numberPlane.c2p(0, 0), numberPlane.c2p(2, 1), buff=np.sqrt(5), color=YELLOW
        )
